#!/usr/bin/env python3
"""Plan a CEML_RA research loop packet from durable thread memory."""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description=(
            "Create a dry-run or durable Research Loop Packet. This does not "
            "mutate research_thread, Slack, Scout, KG/RAG stores, or runtime services."
        )
    )
    ...
    parser.add_argument(
        ...
        help="Research loop trigger type.",
    )
    parser.add_argument("--trigger-summary", required=True, help="Korean-first summary of why this loop is being planned.")
    parser.add_argument(
        "--artifacts-dir",
        type=Path,
    ...
    parser.add_argument(
        "--execute",
        action="store_true",
        help="Write loop packet artifacts. Without this, preview only.",
    )
    return parser
